...
在 MiniClaudeCode 启动时，将项目中的所有 SKILL name 和 description 注入到 SYSTEM_PROMPT
"""
...
def _parse_frontmatter(text: str) -> tuple[dict, str]:
    """解析 SKILL.md 的 YAML 前置元数据（frontmatter），返回 (元数据字典, 正文)。"""
    # 没有 frontmatter 标识符，全文视为正文
    if not text.startswith("---"):
        return {}, text

    # 按 --- 切分，最多切 2 次，得到三部分：空、YAML块、剩余正文
    parts = text.split("---", 2)
    # 切不出来三段，说明格式残缺（比如只有一个 ---），按无元数据处理
    if len(parts) < 3:
        return {}, text

    # 解析中间那段 YAML，解析失败时容错，元数据置空不影响正文读取
    try:
...
def _scan_skills():
    ...
    # Path.iterdir() 返回对应 Path 下一级的文件夹或者文件
...
# 读取所有 SKILL 内容到 SKILL_REGISTRY 中
_scan_skills()

# 列出存在的所有 SKILL name 和 description
def list_skills() -> str:
...
# 构建一个包含所有 SKILL 描述的 SYSTEM_PROMPT
